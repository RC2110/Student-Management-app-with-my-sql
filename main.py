...
import sys
import sqlite3
from PyQt6.QtCore import Qt # search functionality
import mysql.connector

class Connection:
    def __init__(self, host="localhost", user="root", password= "pythoncourse", database="students"):
        self.host = host
        self.user = user
...
class EditDialog(QDialog):
    ...
    def update_stu(self):
        connection = Connection().connect()
        cursor = connection.cursor()
        cursor.execute("UPDATE records SET Name= %s, Course = %s, Mobile =%s WHERE ID = %s", (self.name.text(),
...
class AddStudent(QDialog):
    ...
    def add_stu(self):
        ...
        connection = Connection().connect()
        cursor = connection.cursor()
        cursor.execute("INSERT INTO records (Name, Course, Mobile) VALUES(%s, %s, %s)", (name, course, mobile)) # first () represents the name of the columns as per database
        connection.commit()
        cursor.close()
        connection.close()
